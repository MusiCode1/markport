...
## 2026-08-02 — תיעוד ההצמדה ל-1.12.7 + תיקון הוראות-התקנה שוברות

הודעת הכשל שנוספה ב-boot watchdog מפנה ל-README — ושם לא היה מה לראות. גרוע מכך:
ה-README הנחה להריץ `update-obsidian-mobile.js` **בלי** `--version`, כלומר למשוך את
הגרסה האחרונה, שהיא 1.13.4 ולא עולה. זו הוראה ששוברת התקנה חדשה.

#### מה בוצע?

**1. `docs/obsidian-version-support.md` — קובץ חדש**

באנגלית, בניגוד לרוב `docs/` — הודעת הכשל באנגלית מפנה אליו וקוראים לא-דוברי-עברית
יגיעו אליו. מכיל: הגרסה הנתמכת ומה רואים אם מנסים חדשה יותר · מה 1.13 הוסיפה
(שער `terms` דרך `App.getInfo()`) · למה לא עוקפים · טבלת המדידה המלאה · מה ישנה
את ההחלטה. אומר במפורש שהשים **אינו מכיל** את המחרוזת ונשלח ריק — בלי להפוך את זה
להוראות-הפעלה.

סעיף **"If you choose to run a newer version anyway"**: מי שמספק את הערך **הוא** זה
שמצהיר — לא הפרויקט ולא מי שכתב את הקוד — והאחריות לעמוד בתנאים היא שלו. מפריד
מפורשות בין שימוש פרטי מול הכספת שלך (הצהרה שאפשר לעשות ביושר) לבין **הגשה לאחרים**,
שהיא בדיוק מה שההצהרה אומרת שדורש אישור מפורש: *"do not do it on the strength of a
flag you set in your own browser"*. ומצוין שאין תמיכה ואין אימות מעבר לשער.

**2. `README.md`**

- callout בראש `## Obsidian Version`: הגרסה הנתמכת, שזו החלטה ולא באג פתוח, וקישור למסמך.
- בלוק הפקודות התהפך — הגרסה הנתמכת ראשונה, ו"האחרונה" מסומנת שלא תעלה.
- **חמש** ההופעות של `update-obsidian-mobile.js` יושרו ל-`--version 1.12.7` (שורות
  114, 155, 158, 165, 267). תיקון חלקי היה משאיר את ה-README סותר את עצמו.

#### בדיקות

`client-mobile` 96/96 · `cloudflare` 40/40.

#### חריגות

- לא נגעתי ב-`AGENTS.md`. הוא מתאר מדיניות patches ולא גרסאות, וה-callout ב-README
  הוא הנקודה שאליה הקוד מפנה.
