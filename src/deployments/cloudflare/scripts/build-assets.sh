...
#
# Moved ahead of the client-only-signals inject below (docs/plans/
# demo-origin-split.md §4 Commit 1): window.__owDemoContent is a hash of THIS
# file's on-disk content, so example-vault.json must exist before that step
# reads it.
...
# window.__owVersion=<src/config/version.json>, AND window.__owDemoContent=
# <hash of example-vault.json> (docs/plans/demo-origin-split.md §4 Commit 1) —
# a channel SEPARATE from __owConfigInjected above on purpose (see index.html
# comment: that one is deepMerge'd + byte-verified by two test suites against
# the shared deploy-config.json — folding either flag into it would land it
# on runtime-server too). boot.js reads __owBackend to short-circuit an
...
# the onboarding footer (DoD#8); it reads __owDemoContent to detect that the
# demo template changed since the visitor's vault was last seeded and
# re-seed the template files (seed-example-vault.js opts.force). Injected in
# BOTH builds (harmless in the default profile — nothing reads it there),
# which is what keeps the two artifacts' only diff to the config line
# (DoD#7). runtime-server never replaces this marker → all three stay
# undefined there — __owBackend defaults to "has a backend" (unchanged),
# __owVersion missing means "hide our version, show only Obsidian's" (boot.js
# handles this explicitly, see installVersionDisplay), __owDemoContent
# missing means the re-seed-on-change check never fires (no /example-vault.json
# there either — same no-op as seedExampleVault's own local-dev guard).
#
# ⚠️ Bare command, NOT inside `if` — same lesson already documented at the
# LiveSync step above (`if node ...; then` swallows exit(1) under `set -e`).
...
DEMO_CONTENT_HASH=$(sha256sum "$PUBLIC_DIR/example-vault.json" | cut -c1-16)
echo "  injecting client-only signals (window.__owBackend, window.__owVersion, window.__owDemoContent)..."
VERSION_PATH="$VERSION_PATH" HTML_PATH="$PUBLIC_DIR/index.html" DEMO_CONTENT_HASH="$DEMO_CONTENT_HASH" node -e '
  const fs = require("fs");
  ...
  const snippet = "<script>window.__owBackend=\"none\";window.__owVersion=" + JSON.stringify(version) + ";window.__owDemoContent=" + JSON.stringify(process.env.DEMO_CONTENT_HASH) + ";</script>";
  fs.writeFileSync(process.env.HTML_PATH, html.replace(marker, snippet));
'
